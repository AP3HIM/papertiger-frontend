...
export default function CommentsSection({ movieId }) {
  ...
  const load = async () => setComments(await fetchComments(movieId));
  ...
  const onSubmit = async (e) => {
    ...
    if (!text.trim()) return;
    try {
      await postComment(movieId, text.trim(), rating || null);
    ...
    <div className="comments-wrapper">
      ...
      {comments.length ? comments.map(c => (
        <div key={c.id} className="comment">
          ...
          <p>{c.text}</p>
          {isLoggedIn && c.user_name === localStorage.getItem("username") && (
            <button onClick={()=>onDelete(c.id)} className="delete-btn">×</button>
          )}
        </div>
      )) : <p>No comments yet.</p>}
    </div>
